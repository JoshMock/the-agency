...
| `mounts` | `[]` | Host directories to mount into the VM at runtime. Each entry is `{ "host": "...", "guest": "...", "readonly"? }`. The `host` path supports a leading `~`. Set `"readonly": true` for a read-only mount (writes fail with `EROFS`). Example: `[{ "host": "~/.config/some-tool", "guest": "/root/.config/some-tool" }]` or `[{ "host": "~/.config/keys", "guest": "/root/.config/keys", "readonly": true }]`. |
